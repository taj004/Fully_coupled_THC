...
def gather(gb, 
    ...
    density_wrap = (
        (mass_density.mass * rho_ad_main - mass_density_prev.mass * rho_ad(p_prev)) / dt 
        + conservation
        ) 
   ...
   #%% The Passive tracer
    ...
    if len(edge_list) > 0:
        ...
        # Some tools we need       
        upwind_tracer_coupling = pp.ad.UpwindCouplingAd(keyword=tracer_kw, edges=edge_list)
   ...
   #%% Next, the (solute) transport equations.
    ...
    div = data_grid["divergence_several"] # pp.ad.Divergence(grid_list, dim=num_aq_components)
...
#%% The last equation is mineral precipitation-dissolution
    ...
    def porosity_res(porosity, minerals):
        ...
        #breakpoint()
        ptn =  data_grid["sum_mat"] * (minerals * mins) + chi_nr - np.ones(gb.num_cells())
