...
init_equil_const = constant_params.ref_equil() * update_param.taylor_app_equil(temp=init_temp)
bc_equil_const = constant_params.ref_equil() * update_param.taylor_app_equil(temp=bc_temp)
...
for g, d in gb:
   ...
   # print(specific_volume)
    dynamic_viscosity = constant_params.dynamic_viscosity()
    ...
    # end if-else
    kk = K * specific_volume.copy() / dynamic_viscosity
    ...
    fluid_density = update_param.rho(init_pressure, init_temp)
    ...
    if g.dim == gb.dim_max():
        ...
        d[pp.PARAMETERS]["grid_params"] = {
            ...
            "all_2_aquatic_at_interface": constant_params.all_2_aquatic_mat(
    ...
    d[pp.STATE].update({
        ...
        pressure: unity, #init_pressure * unity,
        tot_var: init_T.copy(),
        log_var: log_conc,
...
for e, d in gb.edges():
    ...
    edge_t = (
        init_temp * 
        update_param.rho(init_pressure, init_temp) * 
    ...
    d[pp.STATE].update({
        ...
        mortar_temperature_convection: edge_t * 0,
...
# Various matrices that are constant in the simulations
data_2d[pp.PARAMETERS]["grid_params"].update({
    ...
    "mat_to_omega": constant_params.enlarge_mat_2(gb_2d.num_cells, gb.num_cells()),
...
picture_folder_name = "pictures/"+"2D/"+refinement_level 
to_paraview = pp.Exporter(
    gb, file_name="variables", 
...
time_store = np.array([0.1, 1.3, 2.5, 3.9, 4.5])*1e3
j=0
