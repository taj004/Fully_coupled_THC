...
def gather(gb, 
    ...
    if len(edge_list)>0:
        ...
        # The interface flux, 
        interface_flux = (
            v + robin.mortar_discr * (pressure_trace_from_high - pressure_from_low)
            )
    # end if
...
#%% The temperatue equtaion. 
    ...
    if len(edge_list) > 0:
        ...
        # Edge conduction
        temp_wrap += div * conduction_mpfa.bound_flux * mortar_projection.mortar_to_primary_int * q
    ...
    if len(edge_list) > 0:
        # Upstream convection
        convection_coupling = pp.ad.UpwindCouplingAd(keyword=temperature, 
                                                     edges=edge_list)
        ...
        conduction_over_interface = (
            q + robin_temp.mortar_discr * (temp_from_high - temp_from_low)
            )
    # end if
...
#%% The last equation is mineral precipitation-dissolution
    ...
    def porosity_res(porosity, minerals):
        ...
        mins = np.vstack([mm1/d1 * gb.cell_volumes(), mm2/d2 * gb.cell_volumes()]).ravel("F") 
    ...
    def aperture_residual(a,minerals):
        ...
        der_matrix = sps.diags( 
            diagonals=np.zeros(gb.num_cells()), 
            shape=a.jac.shape
            )
        
        mins = np.vstack(
            [constant_params.molar_mass_CaCO3()/constant_params.density_CaCO3() * gb.cell_volumes(), 
             constant_params.molar_mass_CaSO4()/constant_params.density_CaSO4() * gb.cell_volumes()]
            ).ravel("F")
        ...
        #breakpoint()
        aperture_res = a - (
            data_grid["dim_2_mat"] * pp.ad.Ad_array(val=np.ones(gb.num_cells()), jac=der_matrix) 
            + data_grid["dim_1_mat"] * (-data_grid["sum_mat"] * (minerals * mins / surface_area) + constant_params.open_aperture()) 
    ...
    def mat_perm(phi):
        ...
        KC = (1-phi)**(-2.0) * (phi)**(3.0) * k0 * np.power(phi0,-3) * np.power(1-phi0, 2)
        return KC
    ...
    def gb_perm(K,phi,a):
        x = np.zeros(gb.num_cells())
        for g,_ in gb:
            if g.dim == gb.dim_max(): x[0:g.num_cells]=1
        # end loop
    ...
    def taylor(equil,temp):
        ...
        t = 1 + dt * c / constant_params.ref_temp()**2 # First order Taylor approximation of van't Hoffs Equation
